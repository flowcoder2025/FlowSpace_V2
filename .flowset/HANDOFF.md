...
## Active WI
(없음) — **WI-012-1-refactor develop 머지 완료**(PR#12, merge `84ea3d8`, impl `3dac762`). 다음 우선은 **WI-012-2-refactor(S3~S6 cleanup) / WI-013-perf(인덱스, DB승인필요)**.

## ⚠️ 다음 세션 우선 — 남은 P3 부채
- **WI-012는 codex consult D6 따라 분할됨**: WI-012-1(S1+S2 protocol/소켓상수, 배포인프라 — **완료·머지**) / **WI-012-2(S3~S6 cleanup, READY)**.
- 큐(READY, 전부 P3):
  - **WI-012-2-refactor**(cleanup 4건, 전부 저위험·독립):
    - **S3 EventBridge 단일 진입점**: 최상위 `game/index.ts` 배럴이 payload 타입 **4/28만** 재노출 → editor 2파일(editor-system/use-editor, `@/features/space/game/events` alias)·avatar 1파일(sprite-generator, 상대경로 `../../game/events`)이 서브배럴 우회 import 중. → game 배럴이 28 payload **명시 나열** 노출 + 3소비처를 최상위 배럴로 통일 + ESLint `no-restricted-imports`에 `@/features/space/game/events` 차단 하드닝(game/internal 동일모듈 상대경로는 영향 없음 확인 필요). server는 game events 미사용(esbuild 무관).
    - **S4 useScreenRecorder `.catch` 통일**(WI-006 흡수): `audioContext.close()` 4곳 중 L263(startRecording 진입)+L357(stopRecording) `.catch` 누락 → codex D4 권고대로 **둘 다** 처리(4경로 완전 통일). `.catch(()=>{})` no-op(unmount L179/onerror L312 관례). + AudioContext 스텁 + audioTracks 전달 케이스 신규(변이검증 — 현재 4 close 경로 전부 미커버).
    - **S5 messages cursor DRY**(WI-010 흡수): 라우트 **2개**(`[id]/messages/route.ts` 비-admin, `[id]/admin/messages/route.ts` admin)가 cursor/limit/절단 인라인 중복 → `src/lib/pagination.ts`(`parsePageLimit`/`buildCursorPage`) 무확장 치환. **응답 계약 보존**: 비-admin은 `{messages: 매핑, nextCursor, hasMore}`(items.map 유지), admin은 `{messages, nextCursor}` **무변경**(hasMore 미추가 — codex D5).
    - **S6 POST /api/spaces 커버리지**(WI-011 흡수): 현재 403만 커버 → 201/401/400/404 추가(+선택 500·GET500 폴백). **테스트 전용·소스 무수정**. mockPrisma.space.create/template.findUnique 이미 배선. makeSpaceRow는 GET 전용이라 POST 201은 인라인 fixture.
    - 매핑 전수: 워크플로 `wse5p5u6u`(6서브스코프). 설계 codex consult r1: `.flowset/eval-results/WI-012-consult-r1.log`(D1~D6 권고).
  - **WI-013-perf**(`GET /api/spaces` 핫 쿼리용 Space 복합 인덱스 `status`+`updatedAt desc`+`id desc` 마이그레이션 — WI-010 듀얼검증 codex r3+evaluator 공통 defer, **DB 마이그레이션 승인+프로덕션 EXPLAIN 검증 필요**).
