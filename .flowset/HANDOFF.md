...
## Active WI
(없음) — WI-005-fix **develop 머지 완료**(PR#8, merge `a3de864`, impl `e8e714c`). 다음 우선은 **WI-006-fix(useScreenRecorder onerror, P3)** → WI-010/011/012(P3 부채).

## ⚠️ 다음 세션 우선 — WI-006-fix (useScreenRecorder onerror dangling Promise)
- READY, P3 무해(자원 누수·기능 무영향). `useScreenRecorder`의 `recorder.onerror` 경로에서 pending stopRecording resolve 미종결 → 'error' 이벤트 시 Promise 영구 pending. onerror에서도 `pendingStopResolveRef` settle 또는 안전망 타임아웃. WI-002 듀얼검증서 evaluator 신규(발산). **경계 변경 아님 → codex consult 불요**(구현 직행 가능).
- 그다음: WI-010-perf(전역 목록 페이지네이션, P3) / WI-011-test(api·socket 라우트 테스트 하니스, P3) / WI-012-refactor(protocol 순수계약 모듈 분리 + 소켓상수 물리이동 + enforce·assets write-side 중앙화, P3).
