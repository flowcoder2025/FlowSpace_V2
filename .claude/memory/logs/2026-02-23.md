...
## 16:xx - Phase 11: IP-Adapter 일관성 최적화

### Completed
- [x] 커밋 `36d12c5`: 치비 파이프라인 대폭 개선 (하이브리드 걷기 모션 + Rembg + 포즈 최적화)
- [x] IP-Adapter end_at 미주입 버그 발견 및 수정
  - `_meta.parameters`에 `ipadapter_start_at`, `ipadapter_end_at`, `ipadapter_weight_type` 추가
- [x] ComfyUI MCP로 A/B 테스트 13가지 (Tests A~M)
  - linear / style transfer / strong style transfer / K+V / img2img 등 시도
  - **최적: `style and composition`, weight=0.8, end_at=0.5**
- [x] Down = Phase A 레퍼런스 직접 사용 (재생성 제거)
  - CHIBI_GENERATE_DIRECTIONS → CHIBI_IPADAPTER_DIRECTIONS(left/up) + CHIBI_ALL_DIRECTIONS
  - 호출 4회→3회, 77초→55초
- [x] tsc ✅

### In Progress
- [ ] Up 하단 핑크 아티팩트 수정 (Rembg 잔여 배경)
- [ ] 미커밋 상태

### Decisions (최종 — 번복 포함)
- **style and composition 채택**: "style transfer"에서 시작 → A/B 테스트로 "style and composition"이 디테일+방향 모두 최적 확인
- **img2img 접근 거부**: latent에 정면 구조가 박혀서 ControlNet으로 방향 전환 불가
- **K+V embeds_scaling 거부**: 모노크롬 + 아티팩트 발생
- **dpmpp_2m/karras 거부**: IP-Adapter와 조합 시 캐릭터 왜곡
- **Down 재생성 제거**: Phase A 레퍼런스를 Down 기저 프레임으로 직접 사용 → 얼굴/디테일 100% 일치

### Modified Files
- `comfyui-workflows/character-chibi-ipadapter.json` — IP-Adapter 파라미터 3개 추가
- `src/features/assets/internal/constants.ts` — IPADAPTER_DEFAULTS 최적화
- `src/features/assets/internal/processor.ts` — Phase A/B 리팩토링

### Test Results
```
GRADE: PASS
32/32 frames, 0 empty
Row 0 (Down): W=66px — Phase A 레퍼런스 직접 사용 ✓
Row 1 (Left): W=33~35px — style and composition으로 디테일 유지 ✓
Row 2 (Right): W=33~35px — Left mirror ✓
Row 3 (Up): W=51~54px — 후면, 일관성 양호 (하단 핑크 아티팩트 남음)
소요: 54.7초 (이전 77초 대비 30% 단축)
```

### Handoff Notes
- **미커밋**: constants.ts, processor.ts, workflow JSON 변경 커밋 필요
- **Up 핑크 아티팩트**: Rembg가 완전히 배경 제거 못하는 케이스 → Task 11.5
- **현재 최적 설정**: ControlNet 0.85 + IP-Adapter(style and composition, w=0.8, endAt=0.5) + Down=ref 직접

---

## 17:xx - Task 11.6: Down IP-Adapter 전환 + Trellis2 탐색

### Completed
- [x] **Task 11.6: Down도 IP-Adapter 파이프라인으로 전환**
  - 문제: Phase A(chibi-frame)과 Phase B(chibi-ipadapter) 간 캐릭터 스케일 차이
    - Down(Phase A): 캔버스 60% vs Left(Phase B): 35% → 스프라이트시트에서 크기 불일치
  - 수정: `CHIBI_IPADAPTER_DIRECTIONS`에 "down" 추가, `refFrameData` 제거
  - 결과: 높이 통일 (112px stddev=0), Down/Up 폭 유사 (72/71px)
  - tsc ✅, GRADE: PASS, 67.3초
- [x] **이전 세션 JSONL 전체 읽기** — 60개 교환 확인, 세션 맥락 완전 파악
- [x] ComfyUI MCP로 Phase B Down/Left/Up 3방향 테스트 실행 및 비교

### In Progress
- [ ] **Task 11.7: Trellis2+UltraShape 3D 파이프라인** (다음 세션)
  - 접근: 2D 치비 → Trellis2(3D 변환) → 4방향 렌더링 → 100% 일관성
  - 참고: https://www.youtube.com/watch?v=H7gqMnK7wUc
  - 필요: ComfyUI-TRELLIS2 노드 설치, TRELLIS.2 모델, UltraShape
  - RTX 4070 12GB에서 설정에 따라 가능 (사용자 확인)
- [ ] 미커밋 상태 (이전 세션 + 이번 세션 변경 모두)

### Decisions
- **Task 11.4 번복 → Task 11.6**: Down=ref 직접 사용이 스케일 불일치 유발 → Down도 IP-Adapter로 전환
- **IP-Adapter 한계 인식**: 크기는 해결됐지만 Left/Right 캐릭터 디자인 불일치 남음 (투구→둥근머리)
- **Trellis2+UltraShape 탐색 결정**: 2D→3D→2D 파이프라인으로 방향 일관성 근본 해결 시도

### Modified Files
- `src/features/assets/internal/processor.ts` — CHIBI_IPADAPTER_DIRECTIONS에 "down" 추가, refFrameData 제거, Down 직접사용 코드 제거
- `.claude/specs/chibi-pipeline/11-consistency-optimization.md` — Task 11.6/11.7 추가
...
32/32 frames, 0 empty
BBox Height: avg=112px, stddev=0px
Row 0 (Down): W=72~72px, H=112~112px — 정면, 투구+파란 망토 ✓
Row 1 (Left): W=50~50px, H=112~112px — 측면, 디자인 불일치 (둥근머리)
Row 2 (Right): W=50~50px, H=112~112px — Left mirror ✓
Row 3 (Up): W=71~71px, H=112~112px — 후면, 파란 망토 ✓
소요: 67.3초
```

### Handoff Notes
- **다음 세션 최우선**: Trellis2+UltraShape 설치 → 치비 캐릭터 3D 변환 테스트
- **미커밋 파일**: processor.ts, constants.ts, character-chibi-ipadapter.json, specs
- **사용자 요청**: 신규 세션 시작 시 이전 세션 JSONL을 읽어서 대화 맥락 확보하도록

---

## 18:xx - Task 11.8~11.9: 프롬프트 최적화 + LoRA 근본 원인 발견

### Completed
- [x] **커밋 f27fdee**: IP-Adapter 일관성 최적화 (이전 세션 미커밋분 포함)
- [x] **Task 11.8: 프롬프트 최적화 테스트** (MCP + 파이프라인)
  - 퍼플렉시티 피드백 적용: 파라미터 하향(ControlNet 0.65, LoRA 0.75, IP-Adapter 0.65/0.4) + 레이아웃 프롬프트 → **후퇴** (레퍼런스 품질 저하)
  - Danbooru 태그 + Animagine 품질 태그 통합: **W stddev 7.2→0.9px** 대폭 개선
  - 초록 배경 테스트: 방향 정확도↑, 플랫폼 잔상 → `no ground, no shadow` 추가로 해결
  - IP-Adapter 0.65/0.4 → Left가 여성 캐릭터로 변함 (identity 붕괴)
  - **최종**: 프롬프트(Danbooru+품질태그+초록배경) 새 버전 + 파라미터 기존 최적값 유지
- [x] **Task 11.9: LoRA 학습 데이터 근본 원인 분석**
  - 학습 이미지 35장 확인 → **2종 캐릭터 디자인 혼합** (A형: 투구 기사 25장, B형: 흰머리 10장)
  - 측면/걷기 포즈가 B형에만 존재 → Left 생성 시 B형으로 끌려감
  - **이것이 방향 간 캐릭터 불일치의 근본 원인** (IP-Adapter 한계가 아님)

### Decisions
- **퍼플렉시티 파라미터 하향 거부**: IP-Adapter 0.65/endAt 0.4 → identity 붕괴. 0.8/0.5 필수
- **Danbooru 태그 + 품질 태그 채택**: W 균일성 대폭 개선 효과
- **LoRA가 근본 원인 확정**: 프롬프트/파라미터 튜닝은 증상 완화에 불과
- **Trellis2 보류**: LoRA 재학습 vs Trellis2 중 방향 결정 필요

### Modified Files
- `src/features/assets/internal/constants.ts` — 프롬프트/네거티브 전면 교체 (파라미터 원복)
- `.claude/specs/chibi-pipeline/11-consistency-optimization.md` — Task 11.8/11.9 추가
- `~/.claude/projects/.../memory/MEMORY.md` — Mandatory Enforcement 규칙 추가

### Test Results (MCP 직접 테스트 — 최적 조합)
```
Phase A ref: 투구+은갑옷+파란망토 (깔끔)
Down (IP-Adapter): 정면 ✓, 레퍼런스와 거의 동일
Left (IP-Adapter): 측면 ✓, 투구 유지됨, 파란 망토 보임
Up (IP-Adapter): 후면 ✓, 투구+망토 뒷모습, 일관성 양호
```

### Handoff Notes
- **미커밋**: constants.ts 프롬프트 변경 (파라미터는 기존과 동일)
- **다음 작업 선택 필요**: LoRA 재학습 (스타일 LoRA) vs Trellis2 3D 파이프라인
- **교훈**: 문제 원인 추적 시 출력 튜닝 전에 입력(학습 데이터/모델)부터 점검할 것
- **ComfyUI MCP 활용**: 파라미터 테스트는 MCP로 직접 실행 (constants.ts 수정 불필요)
