...
## ~13:xx - 치비 LoRA 학습 Phase 1~4 실행

### Completed
- [x] **Phase 1: kohya_ss 설치** (`C:\Users\User\sd-scripts\`)
  - torch 2.6.0+cu124, xformers 0.0.29.post3, accelerate (fp16), bitsandbytes
  - CUDA: True, RTX 4070 확인
- [x] **Phase 2: 학습 데이터 준비**
  - 학습 데이터 35장 선별 (ComfyUI 261개 치비 프레임 → 품질 점수 + 방향 다양성)
  - WD14 Tagger 자동 태깅 + 트리거워드 `flowspace_chibi` 추가
  - 정규화 이미지 140장 생성 (Animagine XL 3.1 순수, LoRA 없이)
  - 디렉토리: `train_data/chibi_sprites/10_flowspace_chibi/` (35 img+txt)
  - 정규화: `ComfyUI/output/reg/1_chibi/` (140 img+txt)
- [x] **Phase 4: FlowSpace 파이프라인 통합** (6파일)
  - capability-checker.ts: `flowspace-chibi` LoRA 감지 추가
  - constants.ts: CHIBI_LORA_PRIORITY, CHIBI_LORA_FALLBACK, CHIBI_LORA_TRIGGER 상수
  - processor.ts: resolveBestChibiLora() + maybeAddTriggerWord() + lora_name 주입
  - 워크플로우 JSON 3개: `lora_name` 파라미터 추가 (_meta.parameters)
  - tsc ✅ lint ✅ vitest 52/52 ✅

### In Progress
- [ ] **Phase 3: LoRA 학습 실행** (백그라운드 진행 중)
  - Task ID: `b94d465`
  - 총 2100 steps, ~33s/step, ETA ~19시간
  - 학습 중 step ~21/2100, loss ~0.053 안정적
  - PYTHONUTF8=1 필수 (cp949 인코딩 오류)
  - 2 에포크마다 체크포인트: `output/flowspace-chibi-v1-000002.safetensors` ~ `000012`
  - 진행 확인: `tail /c/Users/User/AppData/Local/Temp/claude/C--Team-jane-FlowSpace/tasks/b94d465.output`

### Pending
- [ ] **Phase 5: 검증** (학습 완료 후)
  - LoRA를 ComfyUI loras 폴더에 복사
  - 32프레임 스프라이트시트 생성
  - `npx tsx scripts/analyze-spritesheet.ts <path> 8 4` 실행
  - yuugiri vs flowspace-chibi 비교 테스트

### Decisions
- **sd-scripts `datasets` 복수형**: 최신 sd-scripts는 `[dataset]` → `[[datasets]]` TOML 형식 변경
- **cache_text_encoder_outputs 호환성**: shuffle_caption, caption_dropout_rate와 동시 사용 불가 → VRAM 우선으로 캐시 유지, 셔플/드롭아웃 제거
- **PYTHONUTF8=1 필수**: Windows cp949 인코딩으로 sd-scripts 일본어 출력 깨짐
- **network_train_unet_only**: 최신 sd-scripts는 `--train_unet_only` → `--network_train_unet_only`
- **정규화 이미지 140장**: 목표 150이었으나 실질적 차이 없어 140으로 확정
- **외부 치비 소스 없이 진행**: 자체 파이프라인 데이터만으로 스타일 학습 (스타일 일관성 우선)
- **학습 시간 ~19시간**: 계획 3~4시간 대비 크게 초과 (700 img/epoch × 12 epochs / grad_accum 4 = 2100 steps)

### Modified Files
- `src/features/assets/internal/capability-checker.ts` (수정: flowspace-chibi 감지)
- `src/features/assets/internal/constants.ts` (수정: CHIBI_LORA_PRIORITY 등 3 상수)
- `src/features/assets/internal/processor.ts` (수정: resolveBestChibiLora, maybeAddTriggerWord, lora_name)
...
### New Files (sd-scripts 디렉토리)
- `C:\Users\User\sd-scripts\` (전체 — kohya_ss 클론)
- `C:\Users\User\sd-scripts\train_config.toml` (학습 설정)
- `C:\Users\User\sd-scripts\train_lora.sh` (학습 명령 스크립트)
- `C:\Users\User\sd-scripts\select_training_data.py` (데이터 선별)
- `C:\Users\User\sd-scripts\generate_reg_images.py` (정규화 이미지 생성)
- `C:\Users\User\sd-scripts\train_data/` (학습 데이터 35장 + 캡션)

### Handoff Notes (다음 세션)
- **학습 진행 확인**: `tail -5 C:\Users\User\AppData\Local\Temp\claude\C--Team-jane-FlowSpace\tasks\b94d465.output`
- **학습 완료 후**:
  1. `C:\Users\User\sd-scripts\output\flowspace-chibi-v1-*.safetensors` 중 최적 에포크 선택 (보통 epoch 6~10)
  2. ComfyUI loras 폴더에 복사: `cp output/flowspace-chibi-v1-000008.safetensors C:\Users\User\ComfyUI\models\loras\`
  3. ComfyUI 재시작
  4. Phase 5 검증 실행
- **학습이 너무 길면**: epochs 줄이기 (4로 변경 시 ~6시간) 또는 정규화 이미지 줄이기
- **코드 통합 완료**: Phase 4 코드는 이미 반영됨, LoRA 파일만 배치하면 자동 감지
